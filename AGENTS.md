## Project Overview
Folio is a Tauri desktop ePub reader for macOS and Windows that manages a local library, opens each book in its own reader window, supports highlights and notes, performs paragraph-level OpenRouter bilingual translation, and generates quote-cover images while preserving a Books-inspired reading experience. The stack is fixed: the renderer in `src/` uses React 18.3, TypeScript 5.5, Vite 5.4, Tailwind CSS 3.4, shadcn/ui, Zustand, TanStack Query, React Hook Form, Zod, epub.js, and React Router; the native backend in `src-tauri/` uses Tauri 2.x, Rust, rusqlite, reqwest, tokio, serde, sha2, zip, keyring, minidom, quick-xml, `image`, and `uuid`, with the repo root organized around those two app roots plus top-level build/config files.
...
2. TECH_DESIGN_V2.md — how to build it. NEVER choose a different pattern or library.
3. tasks_v2.md — scope boundary only. Tasks point to the spec, they are not the spec.

If a task description conflicts with PRD.md or TECH_DESIGN_V2.md, the task is WRONG.
Follow PRD.md and TECH_DESIGN_V2.md. Do not use your own judgment to fill gaps —
...
- The component map in TECH_DESIGN_V2.md Section 4c
- The key implementation rules in TECH_DESIGN_V2.md Section 6 and the component patterns in TECH_DESIGN_V2.md Section 7
...
- Tauri 2.x for the desktop shell and Rust IPC boundary
- rusqlite 0.31 for SQLite access
...
- zip 2.3 for bilingual ePub assembly
- keyring 3.3 for native secure storage
- minidom 0.15 + quick-xml 0.36 for ePub metadata parsing
- image 0.25 for cross-platform cover image normalization
- uuid 1.x for cross-platform UUID generation
- SQLite 3 (embedded via rusqlite) as the only database
- Tauri CLI 2.x for build output
- macOS arm64 + x86_64 universal binary and Windows x86_64 as the only targets
- Apple Developer ID signing and Windows code signing in production
...
- `src-tauri/src/platform/` contains platform-specific menu, path, and window-restoration helpers only. Do not spread `#[cfg(target_os = ...)]` logic elsewhere.
...
- `src-tauri/src/secure_store.rs` is the thin native secure-storage wrapper. Do not expose API key material to the renderer.

## Coding Standards
- All implementation must follow TECH_DESIGN_V2.md Section 6 exactly; these rules are not optional.
...
- Keep all OpenRouter calls in Rust only, with the API key read from native secure storage in Rust and never serialized into frontend payloads or events.
- Apply highlights with the exact epub.js annotation pattern defined in TECH_DESIGN_V2.md Section 6.
...
- Preserve the Tauri CSP defined in TECH_DESIGN_V2.md Section 6. Do not loosen it or add unapproved origins.
- Preserve Rust-owned window size and position persistence for library, settings, and per-book reader windows.
- Use loading skeletons, inline retryable errors, and mutation invalidation patterns exactly as described in TECH_DESIGN_V2.md Section 4d and Section 7.

## Forbidden Behaviors
- ❌ Do NOT invent UI layout not shown in the ASCII wireframes in PRD.md Section 5b
- ❌ Do NOT use any library not listed in TECH_DESIGN_V2.md Section 1
- ❌ Do NOT add fields or endpoints not defined in TECH_DESIGN_V2.md Section 3 and Section 5
...
- [ ] Component structure matches TECH_DESIGN_V2.md Section 4c for this page
- [ ] All IPC calls use the exact command names, args, and event contracts from TECH_DESIGN_V2.md Section 5
- [ ] Loading / error / empty states implemented per PRD.md Section 7
- [ ] Component follows the implementation and component patterns in TECH_DESIGN_V2.md Sections 6 and 7
